...
def calc_ce_loss(v_mems, targets):
    loss = nn.CrossEntropyLoss().to(v_mems.device)
    error = loss(v_mems.mean(-1), targets)
    #print(error)
    return error

def calc_tet_loss(v_mems, targets, steps=10):
    loss = nn.CrossEntropyLoss().to(v_mems.device)
    time_bins = v_mems.shape[-1]
    t_per_step = int(time_bins / steps)
    e_tet = 0
    for s in range(steps):
        t_idx = int(s*t_per_step) - 1
        e_tet += loss(v_mems[..., t_idx], targets)
    error = e_tet / steps
    #print(error)
    return error

def calc_combined_loss(v_mems, targets):
    loss_ce_mean = calc_ce_loss(v_mems, targets)
    loss_tet = calc_tet_loss(v_mems, targets)
    error = (loss_ce_mean + loss_tet) / 2
    #print(error)
    return error

def calc_tet_max_loss(v_mems, targets, steps=10):
    loss = nn.CrossEntropyLoss().to(v_mems.device)
    time_bins = v_mems.shape[-1]
    t_per_step = int(time_bins / steps)
    e_tet = 0
    for s in range(steps):
        t_idx = int(s*t_per_step) - 1
        v_max = v_mems[..., :t_idx].max(-1)
        e_tet += loss(v_max, targets)
    error = e_tet / steps
    #print(error)
    return error
...
def mean_classifier(y):
    y = y.mean(-1)
    pred = torch.argmax(y, dim=1)
    return pred

def last_classifier(y):
    y = y[..., -1]
    pred = torch.argmax(y, dim=1)
    return pred

def max_classifier(y):
    y = y.max(-1)
    pred = torch.argmax(y, dim=1)
    return pred

def top_k_mean_classifier(y, k=1):
    y = y.mean(-1)
    _, pred = y.topk(k, dim=1, largest=True, sorted=True)
    return pred

def top_k_last_classifier(y, k=1):
    y = y[..., -1]
    _, pred = y.topk(k, dim=1, largest=True, sorted=True)
    return pred

def top_k_max_classifier(y, k=1):
    y = y.max(-1)
    _, pred = y.topk(k, dim=1, largest=True, sorted=True)
    return pred
