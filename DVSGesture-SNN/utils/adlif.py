...
@torch.jit.script
def LIScript(u_tm1: Tensor, current: Tensor, decay_u: Tensor) -> Tensor:
...
class LI_Neuron(Module):
    ...
    def forward(self, input_tensor: Tensor, states: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        #u_t = decay_u * u_tm1 + (1.0 - decay_u) * current
        u_t = LIScript(u_tm1, current, decay_u)
...
@torch.jit.script
def LIFScript(u_tm1: Tensor, current: Tensor, decay_u: Tensor, thr: Tensor, alpha: Tensor, c: Tensor) -> Tuple[Tensor, Tensor]:
    u_t = decay_u * u_tm1 + (1.0 - decay_u) * current
    u_thr = u_t - thr
    # Forward Gradient Injection trick (credits to Sebastian Otte)
    z_t = torch.heaviside(u_thr, torch.as_tensor(0.0).type(u_thr.dtype)).detach() + (u_thr - u_thr.detach()) * SLAYER(u_thr, alpha, c).detach()
    # Symplectic formulation with early reset
    u_t = u_t * (1 - z_t.detach())
    return z_t, u_t

class LIF_Neuron(Module):
    __constants__ = ["features"]
    features: int
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.features = cfg['n_neurons']
        self.dt = 1.0
        self.thr = torch.tensor(cfg.get('thr', 1.0))
        self.alpha = torch.tensor(cfg.get('alpha', 5.0))
        self.c = torch.tensor(cfg.get('c', 0.4))
        self.tau_u_range = cfg['tau_u_range']
        self.train_tau_u_method = 'interpolation'
    ...
    def forward(self, input_tensor: Tensor, states: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        #
        # u_t = u_t * (1 - z_t.detach())
        z_t, u_t = LIFScript(u_tm1, current, decay_u, self.thr, self.alpha, self.c)

        if len(u_t.shape) == 4:
            u_t = u_t.permute(0, 3, 1, 2)

        return z_t.clone(), u_t
...
@torch.jit.script
def SEAdLIFScript(u_tm1: Tensor, z_tm1: Tensor, w_tm1: Tensor, soma_current: Tensor, decay_u: Tensor, decay_w: Tensor,
                  thr: Tensor, alpha: Tensor, a: Tensor, b: Tensor, c: Tensor, q: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
    u_t = decay_u * u_tm1 + (1.0 - decay_u) * (
            soma_current - w_tm1
    )
    u_thr = u_t - thr
    # Forward Gradient Injection trick (credits to Sebastian Otte)
    z_t = torch.heaviside(u_thr, torch.as_tensor(0.0).type(u_thr.dtype)).detach() + (u_thr - u_thr.detach()) * SLAYER(
        u_thr, alpha, c).detach()
...
class SEAdLIF(EFAdLIF):
    def forward(
        ...
        #u_t, z_t, w_t = SEAdLIFScript(u_tm1, z_tm1, w_tm1, soma_current, decay_u, decay_w,
        #                              self.thr, self.alpha, self.a, self.b, self.c, self.q)
...
class LIF(Module):
    def __init__(
        ...
        self.cfg = {'n_neurons': out_features,
                    ...
                    'c': 0.4,
                    'tau_u_range': [5, 25], }
        self.neuron = LIF_Neuron(self.cfg, device, dtype)
